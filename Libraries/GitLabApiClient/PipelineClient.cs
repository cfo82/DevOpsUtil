...
public sealed class PipelineClient : IPipelineClient
{
    ...
    public async Task<PipelineDetail> GetLatestAsync(ProjectId projectId, Action<PipelineQueryOptions> options = null)
    {
        var queryOptions = new PipelineQueryOptions();
        options?.Invoke(queryOptions);

        string url = _queryBuilder.Build($"projects/{projectId}/pipelines/latest", queryOptions);
        return await _httpFacade.Get<PipelineDetail>(url);
    }
